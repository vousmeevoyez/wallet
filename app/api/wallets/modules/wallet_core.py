    ...
    def __init__(self, source=None, pin=None, destination=None):
        if source is not None:
            ...
            if pin is not None:
                ...
                if pin_status == "INCORRECT":
                    raise UnprocessableEntity(error_response["INCORRECT_PIN"]["TITLE"],
                                              error_response["INCORRECT_PIN"]["MESSAGE"])
                elif pin_status == "MAX_ATTEMPT":
                    raise UnprocessableEntity(error_response["MAX_PIN_ATTEMPT"]["TITLE"],
                                              error_response["MAX_PIN_ATTEMPT"]["MESSAGE"])
                elif pin_status == "LOCKED":
                    raise UnprocessableEntity(error_response["WALLET_LOCKED"]["TITLE"],
        ...
        if destination is not None:
            ...
            if destination_wallet == source_wallet:
                raise UnprocessableEntity(error_response["INVALID_DESTINATION"]["TITLE"],

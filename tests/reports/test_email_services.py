...
def test_prepare_email():
    ...
    to = "to@modana.id"
    subject = "unit test"
    content = {"greeting": "some greeting", "message": "some message"}
    attachment = None
    template = "daily_reports"

    result = prepare_email(sender, to, subject, content, attachment, template)
    assert result
...
def test_send_email():
    ...
    result = send_email(recipients, subject, content, fullpath, template)

...
def test_add_bank_account_success(setup_user_factory, setup_bank):
...
def test_add_bank_account_failed_user_not_found(setup_user_factory, setup_bank):
    """ test add bank account but not found"""
    params = {"label": "sample label", "name": "jennie", "account_no": "1234567891"}
    bank_account = BankAccount(**params)

    with pytest.raises(RequestNotFound):
        result = BankAccountServices(fake_uuid, str(setup_bank.id)).add(bank_account)
...
def test_update_bank_account_success(setup_user_factory, setup_bank):
...
def test_update_bank_account_failed_bank_account_not_found(setup_user_factory,
    ...
    params = {"label": "my label", "name": "jennie", "account_no": "1234567891"}
    with pytest.raises(RequestNotFound):
        result = BankAccountServices(str(user.id), str(setup_bank.id), fake_uuid).update(
            params
        )
...
def test_update_bank_account_failed_bank_not_found(setup_user_factory):
    ...
    params = {"label": "my label", "name": "jennie", "account_no": "1234567891"}
    with pytest.raises(RequestNotFound):
        result = BankAccountServices(str(user.id), fake_uuid, fake_uuid).update(
            params
        )

...
from app.api.models import User, Wallet, WalletLock, Withdraw, IncorrectPin
...
def test_create_wallet_serialize_error(client, setup_user_wallet_va):
    ...
    params = {"label": "wallet_label", "pin": "1"}  # ALPHABET ONLY  # PIN TOO SHOORT
...
def test_get_transactions_details_success(
    ...
    result = get_transaction_details(
        client, source_wallet_id, params, source_access_token
    )
    response = result.get_json()
...
def test_verify_forgot_pin(client, setup_user_wallet_va):
    ...
    result = verify_forgot_pin(client, source_wallet_id, params, source_access_token)
    assert result.status_code == 204

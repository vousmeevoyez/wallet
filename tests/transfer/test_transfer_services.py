...
from app.api.utility.utils import validate_uuid, QR
...
def test_internal_transfer_success(
    setup_user_wallet_va, setup_user_wallet_va_without_balance
):
...
def test_internal_transfer_failed_invalid_id(
    setup_user_wallet_va, setup_user_wallet_va_without_balance
):
    """ test function to create main transaction """
...
def test_internal_transfer_failed_source_not_found(
    setup_user_wallet_va, setup_user_wallet_va_without_balance
):
    """ test function to create main transaction """
    # create sourc wallet first
    ...
    with pytest.raises(RequestNotFound):
        result = TransferServices(
            fake_wallet_id, "123456", destination_wallet_id
        ).internal_transfer(params)


def test_internal_transfer_failed_source_locked(
    setup_user_wallet_va, setup_user_wallet_va_without_balance
):
    """ test function to create main transaction """
    source_access_token, source_user_id, source_wallet_id = setup_user_wallet_va
...
def test_internal_transfer_failed_source_wrong_pin(
    setup_user_wallet_va, setup_user_wallet_va_without_balance
):
    """ test function to create main transaction """
    source_access_token, source_user_id, source_wallet_id = setup_user_wallet_va
...
def test_internal_transfer_failed_source_max_wrong_pin(
    setup_user_wallet_va, setup_user_wallet_va_without_balance
):
    """ test function to create main transaction """
    source_access_token, source_user_id, source_wallet_id = setup_user_wallet_va
...
def test_internal_transfer_failed_insufficient(
    setup_user_wallet_va, setup_user_wallet_va_without_balance
):
    """ test function to create main transaction """
    # create sourc wallet first
...
def test_internal_transfer_failed_destination_not_found(
    setup_user_wallet_va, setup_user_wallet_va_without_balance
):
    """ test function to create main transaction """
    source_access_token, source_user_id, source_wallet_id = setup_user_wallet_va
...
def test_internal_transfer_failed_destination_locked(
    setup_user_wallet_va, setup_user_wallet_va_without_balance
):
    """ test function to create main transaction """
    # create sourc wallet first
...
def test_internal_transfer_failed_destination_source_same(
    setup_user_wallet_va, setup_user_wallet_va_without_balance
):
    """ test function to create main transaction """
    # create sourc wallet first
...
def test_external_transfer(mock_bank_transfer, setup_user_wallet_va_bank_acc):
    ...
    result = TransferServices(source_wallet_id, "123456").external_transfer(params)

    transaction = Transaction.query.all()
    assert len(transaction) > 0
    payment = Payment.query.all()
    assert len(payment) > 0
    # make sure it create quota usage entry!
    quota_usage = \
        QuotaUsage.query.filter_by(
            transaction_id=validate_uuid(result[0]["data"]["id"])
        ).first()
    assert quota_usage.usage == -1
    assert quota_usage.status == 0
...
def test_external_transfer_insufficient(setup_user_wallet_va_bank_acc):
    ...
    params = {"amount": 99999999, "destination": source_bank_acc_id, "notes": None}
...
def test_calculate_transfer_fee(setup_bca_bank_account, setup_bni_bank_account):
